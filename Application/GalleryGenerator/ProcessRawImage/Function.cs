...
namespace ProcessRawImage
{
    public class Function
    {
        ...
        readonly HashSet<string> _validImageExtensions = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            "jpg", "jpeg", "png", "gif"
        };
        ...
        public async Task FunctionHandler(S3Event evnt, ILambdaContext context)
        {
            ...
            foreach(var record in evnt.Records)
            {
                ...
                // One possible fix to filter out non-image file types, however relying on
                // extension is really not 100% accurate or future proof - we should instead 
                // catch the exception during moderation and skip the file.
                var fileExt = Path.GetExtension(originalKey).TrimStart('.');
                if (!_validImageExtensions.Contains(fileExt))
                {
                    context.Logger.LogLine($"File {originalKey} does not have a recognized image extension, skipping");
                    continue;
                }
